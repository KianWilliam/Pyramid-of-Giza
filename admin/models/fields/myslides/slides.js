...
function addslidemy(imgname, imgthumb, imgtext)
{
	...
	'<div class="del"><input name="myslidedelete' + counter + '" class="myslidedelete" type="button" value="' + Joomla.JText._('COM_GIZA_REMOVE', 'RemoveSlide') + '" onclick="javascript:removeslide(this.getParent().getParent());" />'+
    ...
    '<a class="modal" href="' + JURI + 'administrator/index.php?option=com_media&view=images&tmpl=component&e_name=myslideimgname' + counter + '" rel="{handler:\'iframe\', size:{x: 570, y: 400}}" >' + Joomla.JText._('COM_GIZA_SLIDESHOWCK_SELECTIMAGE', 'select image') + '</a>'+
	'</div>'+
...
function removeslide(slide) {
	if (confirm(Joomla.JText._('COM_GIZA_SLIDESHOWCK_REMOVE', 'Remove this slide') + ' ?')) {
		slide.destroy();
		counter--;
		storeslide();
	}
...
window.addEvent('domready', function() {
	...
	script.text = "Joomla.submitbutton = function(task){"
			+ "storeslide();"
			+ "if (task == 'pyramid.cancel' || document.formvalidator.isValid(document.id('pyramid-form'))) {	Joomla.submitform(task, document.getElementById('pyramid-form'));"
			+ "if (self != top) {"
